...
Per-class metrics are computed under a one-vs-rest scheme: for each class
$k \in \{\text{Low Risk, High Risk, Unclear Risk}\}$, each study--domain cell is
treated as a binary classification (``class $k$'' vs.\ ``not class $k$''),
yielding four counts:
\begin{itemize}
  \item $\mathrm{TP}_k$: model and ground truth both assigned class $k$;
  \item $\mathrm{FN}_k$: ground truth was $k$, model assigned another class
        (positive missed);
  \item $\mathrm{FP}_k$: model assigned $k$, ground truth was another class
        (false alarm);
  \item $\mathrm{TN}_k$: neither model nor ground truth assigned class $k$.
\end{itemize}
...
\textbf{Consensus failures and model complementarity.}
